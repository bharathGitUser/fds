...
public class FusedQueue implements MessageHandler,Serializable{
	public Vector<Queue<FusedAuxNode>> auxStructure;//:( cannot create array of parameterized objects in java 
	FusedNode[] tos;
	int numOperations =0;
	...
	public FusedQueue(int numPrimaries, int numFaults, int backupId, Vector<Queue<FusedAuxNode>> auxStructure){
	...
	/* dumps the new element at the end of the list for that primary and updates the correct position
	 	in the auxStructure
	*/
	public void add(int value, int primaryId){
		if(Util.debugFlag)System.out.println("-------In Add, v:"+value+" prim:"+primaryId);
		...
		auxStructure.get(primaryId).add(auxNode);		
		//sanityCheck();		
	...
	public void remove(int elementToDelete, int finalElement, int primaryId){
		if(Util.debugFlag)System.out.println("-------In Remove, eToD:"+elementToDelete+" eFin:"+finalElement+" prim:"+primaryId);

		FusedAuxNode auxNode = auxStructure.get(primaryId).remove();
	...
	public Vector<Vector<Integer>> auxStructuresInVectorFormat(){
		...
		Vector<Vector<FusedAuxNode>> auxVectors = new 	Vector<Vector<FusedAuxNode>>();
		for(int i=0; i < numPrimaries;++i)
			auxVectors.add(getAuxInVectorFormat(i));
		...
		while(node != null){
			...
			for(int i =0; i < numPrimaries; ++i){
				...
				if(aux != null)	{
					positions.add(auxVectors.get(i).indexOf(aux));
				}
				else
					positions.add(-1); 
			}
	...
	Vector<FusedAuxNode> getAuxInVectorFormat(int i){
		Vector<FusedAuxNode> v = new Vector<FusedAuxNode>();
		Object[] objArray = auxStructure.get(i).toArray();
		for(int j=0; j < objArray.length;++j){
			FusedAuxNode e = (FusedAuxNode)objArray[j];
			v.add(e);
		}
		return v;
	}
	...
	public  synchronized void handleMessage(ObjectOutputStream outStream, ObjectInputStream inStream, String msg){
        ...
        if(tag.equals("add")){
        	...
        	int primaryId = Integer.parseInt(st.nextToken());
        	long startTime = Util.getCurrentTime();
        	remove(elementToDelete, finalElement, primaryId);
        	long endTime = Util.getCurrentTime();
        	totalUpdateTime = totalUpdateTime+ endTime-startTime;
       }
    ...
    public static void main(String[] args) {
    	...
    	Vector<Queue<FusedAuxNode>> queue = new Vector<Queue<FusedAuxNode>>(numPrimaries); 
    	for(int i =0; i < numPrimaries; ++i){
    		GenericQueue<FusedAuxNode> l = new GenericQueue<FusedAuxNode>();
    		queue.add(l);
    	}
    	
    	FusedQueue backup = new FusedQueue(numPrimaries, numFaults, id,queue); 
