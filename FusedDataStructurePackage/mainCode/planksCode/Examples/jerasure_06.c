...
print_all(int count, int w, int psize,char** array){
	int i=0, j=0; 	
	for(i=0; i < count;++i){
						for (j = 0; j < w; j++) {
										int temp = *(int *)(array+j*psize+i*psize*w); 
										printf("%d ", temp); 
						} 
						printf("\n");
		}
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < k; i++) {
	  ...
	  memcpy(data[i],temp,w*sizeof(int));
	  for(j=0; j < m; j++){	
		  int* matrix_element = talloc(int, w*w);
		  int* temp = bitmatrix+ j*k*w*w+i*w; 
		  for (p = 0; p < w; p++) {
			  for (a = 0; a < w; a++) {
				  matrix_element[p*w+a] = temp[p*k*w+a];
			  }
		  } 
		  jerasure_cauchy_update(data[i],coding[j],matrix_element, w,psize,psize*w); 
	  }
  }

  //jerasure_bitmatrix_encode(k, m, w, bitmatrix, data, coding, w*psize, psize);
  
  printf("Encodings Complete:\n\n");
  print_data_and_coding(k, m, w, psize, data, coding);
	//print_all(k, w, psize,data);
//  print_all(m,w,psize,coding);
  
//  //update test:change d1.  
//  int data_id=0; 
//  printf("Starting update\n\n");
//  char* old_data = talloc(char,psize*w); 
//  for (j = 0; j < w; j++) {
//		for(x = 0; x < psize; x += 4) {
//			memcpy(old_data+j*psize+x, data[data_id]+j*psize+x, sizeof(long));
//		}
//} 
// for (j = 0; j < w; j++) {
//		for(x = 0; x < psize; x += 4) {
//			l = 35+i+x+j;
//			memcpy(data[data_id]+j*psize+x, &l, sizeof(long));
//		}
//}
////
// char* diff = talloc(char,psize*w); 
// galois_region_xor(old_data+w*psize,data[data_id]+w*psize,diff,psize*w);
// int p; 
// int* matrix_element = talloc(int, w*w);
// for(i=0; i < m; i++){	
//         int* temp = bitmatrix+ i*k*w*w+data_id*w; 
//         for (p = 0; p < w; p++) {
//       	  for (j = 0; j < w; j++) {
//       		  matrix_element[p*w+j] = temp[p*k*w+j];
//       	  }
//         } 
//    	jerasure_cauchy_update(diff,coding[i],matrix_element, w,psize,psize*w); 
// }
//  print_data_and_coding(k,m,w,psize,data,coding); 
