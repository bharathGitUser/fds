...
public class MapClient {
    ...
    public MapClient(int numPrimaries, int numFaults){
    ...
    public long recover(Vector<Integer> erasures){
		...
		Vector< Vector<int[]> > backups =  getBackups(erasures);
		...
		for(int fusedNodeNumber =0; fusedNodeNumber < indexInfo.size();++fusedNodeNumber){
			...
			for(int k =0; k < numPrimaries; ++k){
				...
				if(!erasures.contains(k) && (keyOfPrimary != -1)){
					...
					data[k*Fusion.getW()]=original.get(keyOfPrimary);
				}
			}
			try {
			...
			for(int k =0; k < numPrimaries; ++k){
				...
				if(erasures.contains(k) && (keyOfPrimary != -1)){
					HashMap<Integer,Integer> original =  primaries.get(k);
					original.put(keyOfPrimary,data[k*Fusion.getW()]);
				}
			}
    ...
    public Vector<Vector<int[]>> getBackups(Vector<Integer> erasures){
    ...
    public static void main(String[] args) {
    	...
    	MapClient orc = new MapClient(numPrimaries,numFaults);
