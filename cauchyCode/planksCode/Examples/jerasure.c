...
void jerasure_update(int k, int m,int w,int *matrix,char **coding_ptrs,int size,char *old_value, char *new_value, int j){
  ...
  init = 0;
  int h, x; 
  if(dbg)printf("old data:");
  for(h=0;h< size; h+=(w/8)) { 
	  if(dbg)printf(" ");
	  for(x=0;x < w/8;x++){
		  if(dbg)printf("%02x", (unsigned char)old_value[h+x]);
		  //printf("%d", data[i][j+x]);
	  }
  }
  if(dbg)printf("    ");
 
  if(dbg)printf("new data:");
  for(h=0;h< size; h+=(w/8)) { 
	  if(dbg)printf(" ");
	  for(x=0;x < w/8;x++){
		  if(dbg)printf("%02x", (unsigned char)new_value[h+x]);
		  //printf("%d", data[i][j+x]);
	  }
  }
  if(dbg)printf("\n");
  
  sptr = old_value;dptr = new_value;

  diff = talloc(char, sizeof(long));
  galois_region_xor(sptr,dptr,diff,size);
  
  //diff = new_value - old_value
  if(dbg)printf("after xor:",i);
  for(h=0;h< size; h+=(w/8)) { 
	  if(dbg)printf(" ");
	  for(x=0;x < w/8;x++){
		  if(dbg)printf("%02x", (unsigned char)diff[h+x]);
	  }
  }
  if(dbg)printf("\n");
  ...
  for(i =0; i <m; i++){
	switch (w) {
		case 8:  galois_w08_region_multiply(diff, matrix[i*k+j], size, dptr, init); break;
		case 16: galois_w16_region_multiply(diff, matrix[i*k+j], size, dptr, init); break;
		case 32: galois_w32_region_multiply(diff, matrix[i*k+j], size, dptr, init); break;
	}
	/*
	printf("after mul :");
	for(h=0;h< size; h+=(w/8)) { 
...
void jerasure_update_single_code(int k, int m,int w,int mat_element,char *code,int i, int size,char *old_value, char *new_value, int j){
...
//	  if(dbg)jerasure_print_matrix(matrix,m,k,w);
	  ...
	  //dptr = m_i_j * (d_j' - d_j)
	  switch (w) {
			case 8:  galois_w08_region_multiply(diff, mat_element, size, dptr, init); break;
			case 16: galois_w16_region_multiply(diff, mat_element, size, dptr, init); break;
			case 32: galois_w32_region_multiply(diff, mat_element, size, dptr, init); break;
	  }
